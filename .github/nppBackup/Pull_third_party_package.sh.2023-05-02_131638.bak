...
# Package-url：插件编译源代码的makefile文件在仓库根目录下为仓库根目录的浏览器地址栏内的地址，在其他目录时为trunk or branch/branch-name/Package-name，源码路径中的tree/master 替换成 trunk;源码路径中的tree/branch 替换成 branch/branch-name
# 例：
